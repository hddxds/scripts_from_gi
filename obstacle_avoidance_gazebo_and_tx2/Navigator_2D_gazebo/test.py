...
class Navigator:
    ...
    def keep_navigating(self):
        ...
        while 1:
            ...
            while not self.reachTargetPositionDiscrete(end_pos, 4) \
                ...
                self.path = self.algo.find_path(self.get_local_pose_d(), self.driver.get_obstacles_around())
                t2 = time.time()
                ...
                if not self.path:
                    ...
                    self.bresenham_check_path = self.path_prune.path_pruning_bresenham3d(self.collear_check_path,
                                                                                         self.driver.get_obstacles_around())

                    # self.bresenham_check_path = self.collear_check_path
                    print("Path finished processing!")

                    # publish raw path, the actual flight path will defer from published raw path
                    # (1, 0, 0) is RED in RGB order
    ...
    def ros_thread(self):
        ...
        self.octomap_cells_vis = rospy.Subscriber("/octomap_point_cloud_centers", PointCloud2,
                                                  self.octomap_update_callback)
...
if __name__ == '__main__':
    nav = Navigator(save_pts=False)

    nav.set_target_position((0, -3.5, 1))
    nav.keep_navigating()
