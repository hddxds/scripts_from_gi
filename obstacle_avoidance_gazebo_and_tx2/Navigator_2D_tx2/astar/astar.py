...
class Node(object):
    ...
    def move_cost(self, p):
        return 1 if self.point[2] == p[2] else astar_config.astar_config['z_move_cost']
...
class A_star(object):
    ...
    def find_path(self, start_pos, obstacle_pos_list):
        ...
        center_point_can_go = self.is_valid(self.end_pos, obstacle_pos_list)
        path = []
        count = 0
        ...
        while self.openlist:
            ...
            # tell if arrived
            if self.__success(current_obj, center_point_can_go):
