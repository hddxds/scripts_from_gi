...
def do_task(task_list, id):
    ...
    if task_list[id][0] == "gps":
        ...
        if gps_distance(global_gps, gps) < 0.015:
...
if __name__ == '__main__':
    ...
    while not Enter_offboard_result:
        ...
        Enter_offboard_result = flightModeService(custom_mode = 'OFFBOARD')
    if(Enter_offboard_result == True):
        pring("Entered offboard!")
        set_ever_entered_offboard(True)
    ...
    task_id = 0
    while not rospy.is_shutdown() and not get_remote_control():
        
        armService(True)
        Enter_offboard_result = flightModeService(custom_mode = 'OFFBOARD')
        if Enter_offboard_result == False:
            pass
            #print "failed!"
        else:
            pass
