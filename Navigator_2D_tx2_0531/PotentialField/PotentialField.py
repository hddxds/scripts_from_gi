...
class PotentialField3D:
        ...
        for i in x_range:
            for j in y_range:
                for k in z_range:
                    ...
                    # for obstacle in self.obstacle_3d[0:20]:
                    #
                    #     if (p == obstacle).all():
                    #         continue
                    #
    ...
    def cluster_obstacles(self):
        ...
        e = time.time()
        print(len(self.obstacle_3d))
        print(len(self.obstacle_clusters.values()))
    ...
    # def points_to_obstacle_distance(self, point):
    #
    #     for key in self.obstacle_clusters.keys():
    #         ob_center = self.obstacle_clusters[key][0]
    #         distance = np.linalg.norm(ob_center - p)
...
if __name__ == '__main__':
    ...
    s = time.time()
    pf.find_path(np.array([0, 0, 5]), np.array([-10, -50, 8]))
    e = time.time()
