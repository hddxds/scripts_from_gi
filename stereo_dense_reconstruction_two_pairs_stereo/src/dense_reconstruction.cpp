...
void publishPointCloud(Mat& img_left, Mat& dmap,int stereo_pair_id) {
  ...
  for (int i = 0; i < img_left.cols; i++) {
    for (int j = 0; j < img_left.rows; j++) {
      ...
      if (stereo_pair_id == 1)
      {//downward stereo cam.
          point3d_cam = R_downward*point3d_cam+T_downward; //if type error,define Mat R = Mat(3, 3, CV_64FC1),T = Mat(3,1,CV_64FC1);
      }
...
void imgCallback_1(const sensor_msgs::ImageConstPtr& msg_left, const sensor_msgs::ImageConstPtr& msg_right,int stereo_pair_id) {

  out<<"imgCallback 2"<<endl;
  ...
  imshow("LEFT_1", img_left);
  imshow("RIGHT_1", img_right);
  imshow("DISP_1", dmap);
  waitKey(30);
}
...
int main(int argc, char** argv) {
  ...
  int forward_stereo_pair_id = 0;
  message_filters::Synchronizer<SyncPolicy> sync(SyncPolicy(10), sub_img_left, sub_img_right);
  sync.registerCallback(boost::bind(&imgCallback, _1, _2,forward_stereo_pair_id));


  //downward stereo pair.

  message_filters::Subscriber<sensor_msgs::Image> sub_img_down_left(nh,"/stereo_down/left/image_raw",1);
  message_filters::Subscriber<sensor_msgs::Image> sub_img_down_right(nh,"/stereo_down/right/image_raw",1);
  int downward_stereo_pair_id = 1;
  message_filters::Synchronizer<SyncPolicy> sync(SyncPolicy(10), sub_img_left, sub_img_right);
  sync.registerCallback(boost::bind(&imgCallback_1, _1, _2,downward_stereo_pair_id));
